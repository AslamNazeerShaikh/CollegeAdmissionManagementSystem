...
**As-built Linux fix pass (2026-09-19, Fedora/X11, all 4 apps launched + screenshotted into `Images & Documents/apps/`):** Avalonia now mirrors the Flutter/Tauri reference — titlebar carries search + 2 CTAs only (stats strip deleted, it forced horizontal scrolling), Pipeline kanban replaced with stage tabs (`SelectedStage` + `LaneForStage` in `CrmViewModel`) + vertical list, threshold 900→760dp, 248 nav + 320 rail, window 1100×700 default / 800×600 min / resizable, top bar on an `Auto` row so it can never paint over content, `RequestedThemeVariant="Light"` (system Dark made unstyled nav/tab text white-on-light = invisible). Uno's Linux crash fixed with `<SkiaSharpVersion>4.152.0</SkiaSharpVersion>` (SDK floored `libSkiaSharp` at m119, managed 4.152 refused it); window resizable via `AppWindow`; Courses `GridView` owns its scroll (virtualization back on); dialogs `MaxWidth 480`; splash navigation cancellation-safe. Flutter (`analyze` clean, 10/10 tests) and Tauri (web tests 5/5) unchanged — reference behavior confirmed by screenshot.

**As-built Uno CRM rewrite (2026-09-19, same session):** the legacy Java-era UI is gone from Uno — `Splash/MainMenu/Courses/Registration` pages, `Courses/Registration` ViewModels, 6 legacy images, 3 Lottie JSONs and the `Lottie` UnoFeature all deleted. Uno is now the same CRM as the other three: `CrmShellPage` (titlebar + SplitView nav + content + rail), ported `CrmModels`/`Course`/`CrmViewModel` in `Uno.Core`, CRM brushes in `App.xaml`, section switching via `x:Load`, active states painted explicitly in code-behind (WinUI has no class bindings; theme ToggleButton chrome would tint surfaces purple). Linux lessons: `UNO_DISPLAY_SCALE_OVERRIDE` defaults to `1.0` in `Desktop/Program.cs` (XWayland's `Xft.dpi: 192` doubled the UI vs siblings); x:Load-unloaded named elements are null in `ApplyLayout` (guarded + post-navigation repaint); item `Button`s need `HorizontalContentAlignment="Stretch"` or table rows cascade; forced `ApplicationTheme.Light`. All 5 sections verified by screenshot, 0 exceptions, tests 5/5.
