...
- **Java archival + tri-app parity audit (2026-09-14):** legacy Android-native app moved `app/` → `src/AndroidJava/app/` via `git mv` (history preserved), with its full Gradle project (`build.gradle`, `settings.gradle`, `gradle.properties`, `gradlew`, `gradle/`) consolidated under `src/AndroidJava/` so it builds self-contained from that folder (`:app` → `app/`); root `README.md` + new `src/AndroidJava/README.md` record active (`src/AvaloniaUi/` shipping, `src/PlatformUno/` retained port) vs archived status. Java kept as behavior/reference spec — do not delete until Phase 5 backend + offline-PDF replacements land in the .NET apps.
- **AndroidJava toolchain modernization (2026-09-14):** `src/AndroidJava` rebuilt on the `MyDemoApp` reference — Gradle `9.6.0`, AGP `9.4.0` (version catalog `gradle/libs.versions.toml`), `minSdk 31` (Android 12), `targetSdk`/`compileSdk 37`, Java 11, `google()` + `mavenCentral()` only (dead `jcenter()` dropped); PDF viewer swapped to maintained `com.github.mhiew:android-pdf-viewer:3.2.0-beta.1` (same API, `barteksc` only existed on jcenter); `switch` on `R.id` → `if-else` (AGP 8+ non-final res IDs); R8 release fixes (real `easing`/`nineoldandroids` deps + OkHttp `-dontwarn`s). Verified: `:app:assembleDebug`, `:app:assembleRelease`, `:app:testDebugUnitTest` all green.
