...
public partial class RegistrationViewModel : ViewModelBase
{
    ...
    {
        await File.WriteAllTextAsync(DraftPath("draft.json"),
            JsonSerializer.Serialize(ToData(), new JsonSerializerOptions { WriteIndented = true }));
    ...
    private async Task SubmitAsync()
    {
        ...
        await File.WriteAllTextAsync(DraftPath($"registration-{DateTime.Now:yyyyMMdd-HHmmss}.json"),
            JsonSerializer.Serialize(ToData(), new JsonSerializerOptions { WriteIndented = true }));
        StatusMessage = "Registered. Details saved on this device.";
    }
